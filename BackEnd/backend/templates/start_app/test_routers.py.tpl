...
from backend.apps.{{ app_name }}.models import {{ model_name }}
...
@pytest.mark.anyio
async def test_list_{{ model_name_lower }}_filter_status(client_with_token: AsyncClient):
    await {{ model_name }}.filter(id__lte=10).update(status=False)
    params = {{ '{' }}"status": False}
    resp = await client_with_token.get('/{{ model_name_lower }}', params=params)
    assert resp.status_code == 200
    assert resp.json()['code'] == 0
    ...
    for {{ model_name_lower }} in resp.json()['result']['items']:
        assert {{ model_name_lower }}['{{ model_name_lower }}Status'] is False

@pytest.mark.anyio
async def test_{{ model_name_lower }}_item_filter_create_time_end(client_with_token: AsyncClient):
...
@pytest.mark.anyio
async def test_list_{{ model_name_lower }}_filter_create_time_start(client_with_token: AsyncClient):
