...
from .schemas import {{ model_name }}Create, {{ model_name }}Detail, {{ model_name }}Info, {{ model_name }}Update, {{ model_name }}Filter
...
@router.put('/{{ '{' }}{{ model_name_lower }}_id}/status', response_model=Union[SuccessResp, FailResp], summary='改变 {{ model_name_lower }} 的状态',
...
async def list_{{ model_name_lower }}(pg: PageSizePaginator = Depends(PageSizePaginator(max_size=50)), filters:{{ model_name }}Filter=Depends({{ model_name }}Filter)):
...
@router.get('/{{ '{' }}{{ model_name_lower }}_id}', response_model=Union[SingleResp[{{ model_name }}Detail], FailResp], summary='查看 {{ model_name_lower }} 详情',
...
@router.put('/{{ '{' }}{{ model_name_lower }}_id}', response_model=Union[SuccessResp, FailResp], summary='修改 {{ model_name_lower }}',
status_code=status.HTTP_200_OK,
