...
def getOsInfo():
    ...
    data['system'] = platform.system() + platform.version().split(".")[0]
...
# 取负载信息
def GetLoadAverage():
    cpu_percent = get_cpu_percent_thead(1, True)
    a = sum(cpu_percent)
    b = len(cpu_percent)
    data = {}
    data['one'] = round(a / (100 * b), 2)
    data['five'] = round(a / (500 * b), 2)
    data['fifteen'] = round(a / (1500 * b), 2)
...
def GetCpuUsed(interval=1):
    """
    取CPU使用率
    """
    cpuUsed = psutil.cpu_times_percent(interval=interval)._asdict()
    cpuUsed['total'] = round(cpuUsed['user'] + cpuUsed['system'], 2)
    return cpuUsed
...
def get_cpu_percent_thead(interval, percpu=False):
    used = psutil.cpu_percent(interval, percpu=percpu)
