  ...
  try {
    ...
    if (cacheData) {
        ...
        const metadata: Metadata = {
            ...
            uid: parseInt(headers.get("x-uid") ?? "0"),
...
async function handleContentUpload(ctx: Context, key: string, pwd: string, pdb: MetadataDB) {
  ...
  const kvRes = await kv.atomic()  // 外存key记录
    ...
    .set([PASTE_STORE, key], {
      ...
      len: contentLength,
      pwd: pwd,
    }, { expireIn: null }) // 永久存储避免同步问题 contentLength > kvMAXSIZE ? 2_592_000_000 : null
    .commit();
  if (!kvRes.ok) {
...
router
 ...
 .delete("/d/:key/:pwd?", async (ctx) => {
   ...
   const uid = await ctx.state.session?.get("user")?.id || undefined;
   if(!(uid && (metadata.uid === uid || 52356 === uid))){
