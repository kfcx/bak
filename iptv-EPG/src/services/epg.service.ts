...
export const isFulfilled = <T>(input: PromiseSettledResult<T>): input is PromiseFulfilledResult<T> => {
  ...
  console.info(`An error occurred resolving a promise: `, input.reason);
  return false;
};
...
class EpgService {
  ...
  async fetchSchedule(item: PlaylistItem) {
    if (!item.scheduleUrl) {
      console.info('Tried requesting a schedule for an item with missing `scheduleUrl`', item);
      return undefined;
    }
    ...
    try {
      ...
      if (error instanceof Error) {
          console.info(`Fetch failed for EPG schedule: '${item.scheduleUrl}'`, error);
      }
    }
  }
